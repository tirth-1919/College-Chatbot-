...
export default function CollegeRegisterPage({ onBackToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await apiClient.registerCollege(formData);
      setSubmittedApp(res);
    } catch (err) {
  ...
  const handleCheckStatus = async (e) => {
    ...
    try {
      const res = await apiClient.getRegistrationStatus(queryAppId.trim());
      setStatusResult(res);
    } catch (err) {
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'radial-gradient(ellipse 80% 60% at 50% -20%, rgba(11,10,62,0.8) 0%, transparent 70%), var(--bg-primary, #070912)',
      padding: '40px 20px',
      display: 'flex',
      ...
      <div style={{ width: '100%', maxWidth: 840, marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.06)', borderRadius: 10, padding: 4 }}>
          <button
            ...
            style={{
              background: activeTab === 'register' ? 'var(--ait-accent, var(--ait-accent))' : 'transparent',
          ...
          <button
            ...
            style={{
              background: activeTab === 'status' ? 'var(--ait-accent, var(--ait-accent))' : 'transparent',
              border: 'none', color: '#fff', padding: '8px 16px', borderRadius: 8,
              fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s'
            }}
          >
      ...
      <div style={{ width: '100%', maxWidth: 840 }}>
          ...
          <div className="glass-card" style={{ padding: '36px 40px' }}>
            <div style={{ textAlign: 'center', marginBottom: 30 }}>
              <div style={{
                ...
                margin: '0 auto 16px', boxShadow: '0 0 25px rgba(47,124,246,0.15)'
              }}>
                <Building2 size={26} color="var(--ait-accent)" />
              </div>
              <h1 style={{ fontSize: '1.6rem', fontWeight: 800, marginBottom: 8, letterSpacing: '-0.02em' }}>
                Register Your College
              </h1>
              <p style={{ color: 'var(--text-muted, #8b96b5)', fontSize: '0.875rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {/* Institution Details */}
              <div>
                <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--ait-accent)', marginBottom: 14, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
                  1. Institution Details
                </h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div style={{ gridColumn: 'span 2' }}>
                    <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-muted, #8b96b5)', marginBottom: 6 }}>
        ...
        {submittedApp && (
          <div className="glass-card" style={{ padding: '48px 40px', textAlign: 'center', maxWidth: 600, margin: '0 auto' }}>
            ...
            <p style={{ color: 'var(--text-muted, #8b96b5)', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: 28 }}>
              Your college registration request for <strong>{submittedApp.college_name}</strong> has been submitted successfully.
            </p>

            <div style={{
              background: 'rgba(23,23,23,0.8)', border: '1px solid rgba(47,124,246,0.3)',
              borderRadius: 12, padding: '24px', marginBottom: 28, textAlign: 'left'
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted, #8b96b5)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Application ID
                  </div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--ait-accent)', letterSpacing: '0.05em' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-subtle, rgba(255,255,255,0.07))', paddingTop: 14 }}>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-muted, #8b96b5)' }}>Status:</span>
                <span style={{
                  background: 'rgba(234,179,8,0.15)', color: '#facc15',
                  padding: '4px 10px', borderRadius: 9999, fontSize: '0.8rem', fontWeight: 700
                }}>
                  {submittedApp.status || 'PENDING SUPER ADMIN APPROVAL'}
                </span>
              </div>
            </div>

            <p style={{ color: 'var(--text-dim, #4a5578)', fontSize: '0.85rem', marginBottom: 28 }}>
          ...
          <div className="glass-card" style={{ padding: '36px 40px', maxWidth: 640, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              <div style={{
                ...
                background: 'rgba(47,124,246,0.15)', border: '1px solid rgba(47,124,246,0.3)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px'
              }}>
                <Search size={24} color="var(--ait-accent)" />
              </div>
            ...
            {statusResult && (
              <div style={{
                background: 'rgba(23,23,23,0.8)', border: '1px solid rgba(255,255,255,0.1)',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'var(--ait-accent)', fontWeight: 600 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: '0.85rem', color: 'var(--text-muted, #8b96b5)', borderTop: '1px solid var(--border-subtle, rgba(255,255,255,0.07))', paddingTop: 14 }}>
                  <div>
                    <span style={{ color: 'var(--text-dim, #4a5578)', display: 'block', fontSize: '0.75rem' }}>Application ID</span>
                    <strong style={{ color: '#fff' }}>{statusResult.application_id}</strong>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-dim, #4a5578)', display: 'block', fontSize: '0.75rem' }}>Website</span>
                    <a href={statusResult.official_website} target="_blank" rel="noreferrer" style={{ color: 'var(--ait-accent)', textDecoration: 'none' }}>
                      {statusResult.official_website}
                    </a>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-dim, #4a5578)', display: 'block', fontSize: '0.75rem' }}>Contact Person</span>
                    <span style={{ color: '#fff' }}>{statusResult.contact_person}</span>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-dim, #4a5578)', display: 'block', fontSize: '0.75rem' }}>Contact Email</span>
